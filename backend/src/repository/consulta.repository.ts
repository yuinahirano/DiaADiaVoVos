...
export class ConsultaRepository {
  ...
  async criar(dados: Omit<IConsulta, "id">): Promise<ResultSetHeader> {
    const sql = `INSERT INTO consulta 
            (nome_medico, horario, local_consulta, id_idoso, data, descricao)  
              VALUES (?,?,?,?,?,?);`;
    const values = [
      dados.nomeMedico,
      dados.horario,
  ...
  async Compareceu(id: string): Promise<ResultSetHeader> {
    const sql = "UPDATE consulta SET compareceu = TRUE WHERE id=?;";
    const values = [id];
    const [rows] = await db.execute<ResultSetHeader>(sql, values);
    return rows;
  }
